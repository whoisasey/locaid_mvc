...
const App = () => {
  ...
  if (data === []) {
    ...
    return (
      <Router>
        <ScrollArrow />
        <Nav props={data} setSearch={setSearch} Desktop={Desktop} categories={categories} locations={locations} setSearchVal={setSearchVal} searchSpace={searchSpace} searchVal={searchVal}/>

          <Switch>
          <Route exact path="/" render={() => <Dashboard props={ data}/>} />
          <Route path="/all" render={() => <AllPages toRender={toRender} search={search} locations={locations}/> } />
